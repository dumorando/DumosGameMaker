...
javascript.javascriptGenerator.forBlock['time_waitforseconds'] = function(block, generator) {
    var field_waitfor_indicator = block.getFieldValue('WAITFOR_INDICATOR');
    var value_seconds_input = generator.valueToCode(block, 'SECONDS_INPUT', javascript.Order.ATOMIC);
    var field_seconds_indicator = block.getFieldValue('SECONDS_INDICATOR');
    var statements_codeafterseconds = generator.statementToCode(block, 'CODEAFTERSECONDS');
    // TODO: Assemble javascript into code variable.
    var code = `wait(${value_seconds_input.toString()}, () => {\n${statements_codeafterseconds}\n})\n`;
    return code;
  };

  javascript.javascriptGenerator.forBlock['time_everyseconds'] = function(block, generator) {
    var field_waitfor_indicator = block.getFieldValue('WAITFOR_INDICATOR');
    var value_seconds_input = generator.valueToCode(block, 'SECONDS_INPUT', javascript.Order.ATOMIC);
    var field_seconds_indicator = block.getFieldValue('SECONDS_INDICATOR');
    var statements_codeafterseconds = generator.statementToCode(block, 'CODEAFTERSECONDS');
  ...
  javascript.javascriptGenerator.forBlock['time_timer'] = function(block, generator) {
    // TODO: Assemble javascript into code variable.
    var code = 'time()';
